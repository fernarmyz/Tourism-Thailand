...
def plotgraph(data,name , chart_title, x_axis_title, y_axis_title):
    """
        plot graph
    """
    dataframe = data
...
def main():
    ...
    for continent in range(len(files)):
        ...
        data_keep[list_continent[continent]] = dataframe.sum().tolist()[2:]
        
        plotgraph(dataframe,name,title, x_title, y_title)
         
        data = pd.DataFrame(data_keep)
    # print(data_keep)
        # sum_data[year].append(data[list_continent[continent]][1].sum()) #for each country === >  data[list_continent[continent]][1]
